...
class RKRangeLayer: CALayer {
    ...
    */
    func drawMarkerInHorizontalDirection(_ marker: RKRangeMarker, at pos: CGFloat, in context: CGContext) {
        ...
        let textAttributes = [
            NSAttributedString.Key.font.rawValue: marker.type.font,
            NSAttributedString.Key.foregroundColor: marker.type.color
            ] as [AnyHashable : NSObject]
        let textSize = NSString(string: marker.text).size(withAttributes: textAttributes as! [NSAttributedString.Key : Any])
        ...
        if marker.value >= rangeEnd || marker.type.labelVisible {
            NSString(string: marker.text).draw(in: markerTextRect)
